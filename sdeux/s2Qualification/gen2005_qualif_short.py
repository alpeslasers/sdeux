...
from sdeux.s2Qualification.measurement_scripts.Laser_output_ON import execute_laser_on_measurement
from sdeux.s2Qualification.measurement_scripts.mode_AB_pulsing import execute_AB_measurement
from sdeux.s2Qualification.measurement_scripts.mode_A_pulsing import execute_mode_A_measurement
from sdeux.s2Qualification.measurement_scripts.mode_B_pulsing import execute_mode_B_measurement
from sdeux.s2Qualification.measurement_scripts.mode_C_pulsing import execute_mode_C_measurement
from sdeux.s2Qualification.measurement_scripts.mode_internal_pulsing import execute_internal_measurement
from sdeux.s2Qualification.measurement_scripts.uptime_counters import execute_time_counters_measurement
from sdeux.s2Qualification.measurement_scripts.with_wfg_measurement import execute_wfg_measurement
...
if __name__ == '__main__':
    ...
    with dms.acquire_instruments(oscillo_th, ps_th, ju_th, wg_th, mm_th):
        ...
        try:
            ...
            #     save_measurement(get_s2_name(s2), data)
            #
            oscillo.cursor_position(0) # position du curseur à 0 (delay)
            # =========================== INTERLOCKS ===================================================================
            # IN ARM ON
            ...
            jura.set_relays({'IN_ARM': 'OFF',
                             ...
                             'GND': 'OFF'})
            sleep(5)
            s2config = dict(pulsing_mode='internal', voltage=5, pulse_period=15000, pulse_width=300, current_limit=20)
            s2.set_settings(**s2config)
            oscillo.set_settings(channel=2, volt_scale_chan=5, offset_chan=0, chan_trig=2, time_scale=3e-6)
            sleep(2.0)
            data = execute_internal_measurement(s2, s2config, oscillo, power_supply, jura)
